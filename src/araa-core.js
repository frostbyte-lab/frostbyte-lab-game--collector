...
export function analyzeAraaEvidence(input = {}) {
  ...
  if (errors.length) findings.push(finding("ARAA-ERROR", "high", "Error capture/analyzer ditemukan", `${errors.length} error tercatat dalam evidence.`, errors.slice(0, 3).map(text).join(" | "), "Buka log detail dan ulangi tahap yang gagal."));
