...
function bodyOf(request) { return request.json().catch(() => ({})); }
...
function requestKey(input) { return String(input.idempotency_key || input.request_id || ""); }
...
function transaction(state, input, amount, event_type, extra = {}) {
  const key = requestKey(input); if (!key) throw Object.assign(new Error("request_id dan idempotency_key wajib diisi."), { code: "INVALID_REQUEST", status: 400 });
...
export async function handleNativeApi(request, env = {}) {
  ...
  let input = {};
  if (request.method === "POST") input = await bodyOf(request);
  if (request.method === "POST" && path === "/session") {
    const playerId = String(input.player_id || "demo-player").slice(0, 80); const session_id = uuid();
    sessions.set(session_id, { session_id, player_id: playerId, expiresAt: Date.now() + 30 * 60 * 1000, status: "active" });
    const state = stateFor(playerId); addHistory(state, "session.create", session_id, input.request_id || null);
    return json({ ok: true, session_id, player_id: playerId, expires_at: new Date(Date.now() + 30 * 60 * 1000).toISOString(), status: "active" });
  }
  let session;
  ...
  try {
    if (request.method === "POST" && path === "/bet") {
      const bet = Number(input.bet); if (!Number.isInteger(bet) || bet < NATIVE_CONFIG.limits.min_bet || bet > NATIVE_CONFIG.limits.max_bet) return fail("BET_OUT_OF_RANGE", "Nilai bet di luar batas.", 422); if (state.balance < bet) return fail("INSUFFICIENT_BALANCE", "Saldo tidak mencukupi.", 422); return json(transaction(state, input, -bet, "bet.authorize", { bet, round_id: uuid() }));
    }
    if (request.method === "POST" && path === "/spin") {
      const bet = Number(input.bet); if (!Number.isInteger(bet) || bet < NATIVE_CONFIG.limits.min_bet || bet > NATIVE_CONFIG.limits.max_bet) return fail("BET_OUT_OF_RANGE", "Nilai bet di luar batas.", 422); if (state.balance < bet) return fail("INSUFFICIENT_BALANCE", "Saldo tidak mencukupi.", 422);
      const roll = (Number(input.seed) || Date.now()) % 10; const win_amount = roll >= 7 ? bet * (roll === 9 ? 5 : 2) : 0; const round_id = uuid(); const result_id = uuid();
      const response = transaction(state, input, -bet + win_amount, "round.spin", { bet, round_id, result_id, result: { result_id, round_id, symbols: [roll % 5, (roll + 1) % 5, (roll + 2) % 5], win_amount, status: "posted" } });
      state.results.set(result_id, response.result); state.rounds.unshift({ round_id, player_id: state.player_id, bet, result: result_id, status: "posted", created_at: timestamp() }); return json(response);
    }
